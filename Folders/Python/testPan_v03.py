...
class TagGroups(QWidget):
    ...
    def __init__(self):
        ...
        def mkButton(sizeX,sizeY,tag, pat=""):
            ...
            self.button.clicked.connect(lambda: tagButtonClick(self,shotTags,shots))
        ...
        def tagButtonClick(self,shotTags,shots):
            ...
            for w in layout_widgets(self.layout):
                ...
                elif wName in ["Layout","Angle"]:
                    for i in range(w.count()):
                        myWidget = w.itemAt(i).widget()
                        myWidget.setChecked(False)
                 

        tagButtonClick(self,shotTags,shots)
    def run(self):
        # Show the form
        self.show()
