...
__credits__ = ["Adrian Pueyo", "Helge Stang", "Eyal Sirazi", "Marco Meyer", "Tony Lyons", "Mark Joey Tang", "Pete O'Connell", "Ivan Busquets","Trixter Folks!!!!!"]
__version__ = "9.0"
...
def go():
    ...
    modes = ['MatchMove','3D Locator','Geometry',"WorldPosition","Deep"]
    ...
    with nuke.Root():
        ...
        def C2T(dialog):
            ...
            if '_X_'+label not in C2Tgroup.knobs(): # if we do create this object first time we will create knobs
                ...
                eGMatrix        = nuke.Tab_Knob('', None, nuke.TABENDGROUP)
                eGTransform     = nuke.Tab_Knob('', None, nuke.TABENDGROUP)
                eGCorner        = nuke.Tab_Knob('', None, nuke.TABENDGROUP)
                eGCard          = nuke.Tab_Knob('', None, nuke.TABENDGROUP)
            ...
            card.setValue('import cardToTrack; cardToTrack.cardCode()')
            corner.setValue('import cardToTrack; cardToTrack.cornerCode()')
            roto.setValue('import cardToTrack; cardToTrack.rotoCode()')
            transform.setValue('import cardToTrack; cardToTrack.transformCode()')
            ...
            threading.Thread(target=calculateCornerPin, kwargs=dict(C2Tgroup=C2Tgroup,label=label,recalculate = recalculate)).start()
            ...
            killAnimation([to1, to2, to3, to4, translate, rotate, scale, uniform_scale])
    ...
    for label in cardTabs:
        with nuke.Root():
            ...
            if translateT.isAnimated():
                stop_event = threading.Event()
                threading.Thread(target=calculateTranslate, kwargs=dict(C2Tgroup=C2Tgroup,label=label)).start()
                while not stop_event.is_set():
                    time.sleep(0.02)
            ...
            if matrix.isAnimated():
                stop_event = threading.Event()
                threading.Thread(target=calculateMatrix, kwargs=dict(C2Tgroup=C2Tgroup,label=label)).start()
                while not stop_event.is_set():
                    time.sleep(0.02)
    ...
    with nuke.Root():
        ...
        for node in nan:
            if 'card_to_track' in node.knobs():
                print('hallo')
                if group_name in node['CardToTrack'].value():
...
def bakeExpressions(knob, start, end, views=None):
    '''
...
def calculateCornerPin(C2Tgroup,label,recalculate):
    '''
    ...
    for one in toes:
        idx = str(toes.index(one)+1)
        one.setExpression('(a'+idx+'.world_matrix.3/-a'+idx+'.world_matrix.11)*input0.width*DummyCam.focal / DummyCam.haperture + input0.width/2 - DummyCam.win_translate.u*input0.width/2',0)
        one.setExpression('(a'+idx+'.world_matrix.7/-a'+idx+'.world_matrix.11)*input0.width*DummyCam.focal / DummyCam.haperture + input0.height/2 - DummyCam.win_translate.v*input0.width/2',1)
...
def calculateTranslate(C2Tgroup,label):
    '''
...
def calculateMatrix(C2Tgroup,label):
    '''
    ...
    if panel.show(): 
        ...
        with nuke.Root():
            ...
            if ask:
                ...
                if C2Tgroup['translate'].isAnimated():
...
def rotoCode():
    ...
    if ask:
        ...
        #kill animation in all not animated
        # for index in range(16):
        #     anim = matrix.animation(index)
        #     if anim.constant():
        #         matrix.clearAnimated(index)
    else:
        nuke.message(message)
...
def toggle_matchmove_stabilise(node):
    ...
    if lab == mm:
        ...
        if node == 'translate':
            ...
            pass
            #ntn['image_aspect'].setValue(0)
            #ntn['image_aspect_out'].setValue(0)
            #ntn['use_output_format'].setValue(0)
        ...
        if node == 'translate':
            ...
            pass
            #ntn['image_aspect'].setValue(1)
            #ntn['image_aspect_out'].setValue(0)
            #ntn['use_output_format'].setValue(1)
