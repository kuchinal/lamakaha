...
def corn3D():
    ...
    if len(u) == 3:  
        ...
        panel.addBooleanCheckBox('Fix flipping', False)
        if panel.show():
            ...
            fixerE = panel.value("Fix flipping")
            ...
            if Axis == False:
                ...
                #################################################
                if fixerE == 1:
                    def fixer(rangeA,rangeB,Axis,Cam,output,refFrame,add):
                        import math
                        first = rangeA                                
                        last = rangeB
                        count =0
                        Backframe = refFrame
                        while Backframe >= rangeA: 
                            #getting the axis position
                            x = Axis['world_matrix'].getValueAt(Backframe,3)
                            y = Axis['world_matrix'].getValueAt(Backframe,7)
                            z = Axis['world_matrix'].getValueAt(Backframe,11)
                            #getting the camera position
                            xc = Cam['translate'].getValueAt(Backframe,0)
                            yc = Cam['translate'].getValueAt(Backframe,1)
                            zc = Cam['translate'].getValueAt(Backframe,2)
                            #getting the camera vector
                            a = Cam['matrix'].getValueAt(Backframe,2)
                            b = Cam['matrix'].getValueAt(Backframe,6)
                            c = Cam['matrix'].getValueAt(Backframe,10)
                            #getting the vector
                            vecx = xc - x
                            vecy = yc - y
                            vecz = zc - z
                            ...
                            if dot <= 0 and count ==0:
                                print Backframe
                                breakx = output['output'].getValueAt(Backframe+add)[0]
                                breaky = output['output'].getValueAt(Backframe+add)[1]
                                count = 1
                            if dot<=0:
                                output['output'].setValueAt(breakx,Backframe+add,0)
                                output['output'].setValueAt(breaky,Backframe+add,1)
                            Backframe = Backframe-1
                        count =0
                        Frontframe = refFrame


                        while Frontframe <= rangeB: 
                            #getting the axis position
                            x = Axis['world_matrix'].getValueAt(Frontframe,3)
                            y = Axis['world_matrix'].getValueAt(Frontframe,7)
                            z = Axis['world_matrix'].getValueAt(Frontframe,11)
                            #getting the camera position
                            xc = Cam['translate'].getValueAt(Frontframe,0)
                            yc = Cam['translate'].getValueAt(Frontframe,1)
                            zc = Cam['translate'].getValueAt(Frontframe,2)
                            #getting the camera vector
                            a = Cam['matrix'].getValueAt(Frontframe,2)
                            b = Cam['matrix'].getValueAt(Frontframe,6)
                            c = Cam['matrix'].getValueAt(Frontframe,10)
                            #getting the vector
                            vecx = xc - x
                            vecy = yc - y
                            vecz = zc - z
                            ...
                            if dot <= 0 and count ==0:
                                print Frontframe
                                print Frontframe-add
                                breakx = output['output'].getValueAt(Frontframe-add,0)
                                breaky = output['output'].getValueAt(Frontframe-add,1)
                                count = 1


                            if dot<=0:
                                output['output'].setValueAt(breakx,Frontframe-add,0)
                                output['output'].setValueAt(breaky,Frontframe-add,1)
                            Frontframe = Frontframe+1
                    
                    

                    add = 1
                    rangeA = nuke.Root().knob('first_frame').getValue()-add
                    rangeB = nuke.Root().knob('last_frame').getValue()+add

                    Axis= nuke.toNode("LU")
                    output = nuke.toNode("P4")
                    fixer(rangeA,rangeB,Axis,Cam,output,refFrame,add)

                    Axis= nuke.toNode("RU")
                    output = nuke.toNode("P3")
                    fixer(rangeA,rangeB,Axis,Cam,output,refFrame,add)

                    Axis= nuke.toNode("RL")
                    output = nuke.toNode("P2")
                    fixer(rangeA,rangeB,Axis,Cam,output,refFrame,add)

                    Axis= nuke.toNode("LL")
                    output = nuke.toNode("P1")
                    fixer(rangeA,rangeB,Axis,Cam,output,refFrame,add)
    ...
    #crop['xpos'].setValue(x+200)
    #crop['ypos'].setValue(y+40)
    #crop.setInput(0,corner)
